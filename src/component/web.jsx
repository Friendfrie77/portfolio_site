...
import xmlPlugin from '../images/web/xmlPlugin.mp4'
...
function Web(){
    ...
    const periodTrackerDes = "For this project, I was tasked to make a simple period tracking application. I used this as an opportunity to get a better understanding of React and the MERN stack in general. At the moment, the site offers basic tracking using the Planned Parnethood's method of tracking found on their website. More features will be added at a later date, once the accuracy of the application can be honed in. Please note, the back-end server is currently running on a free-tier server, so it is slow to log in."
    const toebeansDes = "For this project, I was tasked with making a site using technology that I haven’t used before. To answer that task, I choose to make a full stack site using the Flask framework for a made-up cat sanctuary/adoption center. To handle data I used PostgreSQL, to store users' emails and sign-ups for volunteers. This project allowed me to start learning how to work with databases better, and to better handle JSON data. Please note, the first load of the page might be slow as it is running on an Azure free tier."
    const oopDes = "For this project, I was tasked with making a full application using OOP to make a bike rental app. While the base classes were given, I heavily modified and added to the classes for them to fit the specifications given. I coded and debugged the program over the course of a week. You can find the code in the GitHub link below."
    const sqlDes = 'For this project I was tasked to make a mock SQL database that handled a medical study. I was tasked with making functions and stored procedures that would allow doctors and research staff easily be able to enter in patients into the studies and into the offices database for storing patients. It also had to allow for adding patients to the studies and removing them from studies with reasons that were given in the data. You can see the full SQL script in the GitHub link below.'
    const xmlDes = "For this project, my group and I were tasked with figuring out how to parse XML data and integrate it with the company's WordPress job board. We concluded that the best way to do this was to build a plugin that allows a user to submit an XML link, allows the link to be parsed, gathers any information needed, and saves it to the database. The plugin integrates with the WordPress Job Manager plugin to allow a user to easily manage jobs that are posted via the plugin. The plugin also offers an admin page to change settings and delete XML links."
    return(
        <section className="content" ref={ref} id='web'>
            ...
            <ul className='projects'>
                <Projectcard 
                    projectClass = 'web-zero' 
                    projectTitle = 'Period Tracker (Work in Progress)' 
                    description={periodTrackerDes} 
                    techList = {['React', 'MongoDB', 'Node.js', 'Firebase', 'Redux']} 
                    isVideo = {true} 
                    media = {test} 
                    hasLightbox = {false}
                    linkObj = {{GitHub: 'https://github.com/Friendfrie77/Period_Tracker', LiveSite: 'https://period-tracker-7426c.web.app/', Design: '/PeriodTracker'}}
                ></Projectcard>
                <Projectcard
                    projectClass = 'web-one'
                    projectTitle = 'Toebean Sanctuary'
                    description = {toebeansDes}
                    techList = {['Flask', 'Azure', 'GSAP','PostgreSQL', 'SQLAlchemy']}
                    isVideo = {true}
                    media = {toebeansvideo}
                    hasLightbox = {false}
                    linkObj = {{GitHub: 'https://github.com/Friendfrie77/Web-240-Project', LiveSite: 'https://toebeansanctuary.azurewebsites.net/'}}
                ></Projectcard>
                <Projectcard
                    projectClass = 'web-two'
                    projectTitle = 'Intern Project: JobsJustForYou WordPress XML Parser Plugin'
                    description = {xmlDes}
                    techList = {['PHP', 'SQL', 'WordPress']}
                    isVideo = {true}
                    media = {xmlPlugin}
                    hasLightbox = {false}
                    linkObj = {{GitHub: 'https://github.com/XML-JJFY/xml-plugin'}}
                ></Projectcard>
               <Projectcard
                    projectClass = 'web-three'
                    projectTitle = 'Classwork: Object-Oriented Programming Final'
                    description = {oopDes}
                    techList = {['Python']}
                    isVideo = {false}
                    media = {OOP}
                    linkObj = {{GitHub:'https://github.com/Friendfrie77/CPDM-120-Final'}}
               ></Projectcard>
               <Projectcard
                    projectClass = 'web-four'
                    projectTitle = 'Classwork: SQL Final'
                    description = {sqlDes}
                    techList = {['SQL']}
                    isVideo = {false}
                    media = {SqlFinal}
                    linkObj = {{GitHub: 'https://github.com/Friendfrie77/It-112-Final'}}
               ></Projectcard>
